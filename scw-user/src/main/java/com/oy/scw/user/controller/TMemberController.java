...
import com.oy.scw.user.vo.resp.*;
...
public class TMemberController {
    ...
    public AppResponse<TMember> getMebmerById(@RequestParam("id") Integer id) {
        TMember member = tMemberService.getMemberById(id);
    ...
    public AppResponse<List<UserAddressVo>> address(String accessToken) {
        ...
        List<TMemberAddress> addressesList = tMemberService.listAddress(Integer.parseInt(memberId));

        List<UserAddressVo> addressVoList = new ArrayList<>();

        for(TMemberAddress tMemberAddress :addressesList){
